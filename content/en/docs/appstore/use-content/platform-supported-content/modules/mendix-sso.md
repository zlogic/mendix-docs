...
The Mendix SSO module is typically used when you are collaborating with a small team to prepare the functionality of your app for production. During such an interactive development process, this brings two benefits:
...
In addition, the Mendix SSO module has a default implementation for user administration. This can be used in any Mendix app, but if you want to implement customized user administration, this is also possible. For more information, see the [Customizing Mendix SSO](#customizing) section below.

## Setting Up Mendix Single Sign-On{#setting-up}

These instructions are for apps that did not originally have the Mendix SSO module. For example, if you have an existing app that did not have the Mendix SSO Marketplace module.

{{% alert color="info" %}}
You do not have to follow these steps for apps (for example, app templates) that already have Mendix SSO, or if you are upgrading an existing Mendix SSO module to a newer version.
{{% /alert %}}
...
4. Turn on the **Production** security level and configure **User roles** *User* and *Administrator* to have access to the appropriate **MendixSSO** module roles by performing the following steps:
    ...
    8. Repeat the steps above to add the **MendixSSO.User** module role to the **User** User roles.
        ...
        The app **Security** settings now contain these two additional module roles:
...
{{% alert color="info" %}}By default, the Mendix SSO is not enabled for new apps or apps which do not have the Mendix SSO module. In such scenarios, follow the instructions in the [Setting Up Mendix Single Sign-On](#setting-up) section below.{{% /alert %}}
If you have an app that already has Mendix SSO activated, you can remove it using one of the methods below.
...
3. Delete the **MendixSSO** module from the **Marketplace modules**.
4. Review the **Errors** pane for any other references to **MendixSSO**—there will only be additional errors if the Mendix SSO module has been modified.

### Removing Mendix SSO Java Libraries

The steps above will not remove any Java libraries associated with Mendix SSO.
...
The default Mendix SSO implementation is based on snippets. You can use these snippets in your own pages to customize the administration of the end-users. If you look at how they are used in the default implementation, you can see how to use them on your own pages. The snippets are:
...
{{% alert color="warning" %}}
Mendix recommends that you do not modify the version of Mendix SSO which is in the Marketplace modules section of your app. In the future, you may wish to import a newer version of the module and this will overwrite any changes you make.
{{% /alert %}}
...
5. Move the **MOVE_THIS** folder from **MendixSSO** to the existing module containing your customized user administration entity.
...
{{% alert color="info" %}}
Remember that data that comes from the end-user's Mendix ID via SSO (for example, **EmailAddress**) overwrites any changes you make within your app.
{{% /alert %}}
...
If you want administrators or end-users to be able to see tokens, it is recommended that you add these to the navigation of the app. This prevents them from being included in the main process flows of the app.
...
The TokensOverview page allows administrators to see all tokens that have been issued to end-users of the app.
...
The page can also be used for administration. You can delete tokens that are expired and you can also delete current tokens if they are causing unwanted issues.
