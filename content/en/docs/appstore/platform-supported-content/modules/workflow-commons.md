...
* **TaskAssignment** – This page provides the workflow administrator the ability to manage the assignment and targeting of user tasks based on a selected user. This is typically done when a user is leaving the organization or a group/role, where all the tasks a selected user is working on need to be assigned to another user or unassigned from the departing user. On the page, first select the user whose tasks should be listed. There are two tabs, first is the **Assigned tasks** shows a list of tasks assigned to the selected user, possible actions are reassigning and unassigning the corresponding task(s). Second, **Targeted tasks** shows a list of tasks that the selected user is part of the target user(s), the administrator can retarget these tasks to another user.
