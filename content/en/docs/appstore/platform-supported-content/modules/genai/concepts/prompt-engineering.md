...
* context and information that the model needs in order to follow the instructions
...
The prompts are key components in the interaction with GenAI. When implementing patterns like RAG and ReAct as a developer, you can influence the behavior of the system by modifying the prompt. You need to explain the system how to use the knowledge and functions that are provided, otherwise it might ignore them, act in a different way or start hallucinating.
...
The **system prompt** represents the desired behavior and guidelines of the AI model. It gives the model, for example, a role, tone, ethical regulations, and/or subject specification. This is traditionally set by a prompt engineer. An example of this type of prompt is: *You are a helpful assistant who provides only information about Mendix* – providing direct guidelines on the role, content, and limitations.
...
Depending on the project or use case, adding contextual information to the model might be needed or required. Normally, this information, called **context prompt** or **conversation history**, is sent in the same interaction as the system and user prompt. It captures the historical information of the conversation to maintain coherence with the end user and be context aware. In a chat-bot setup in a Mendix app, this is set by developers within their application in Mendix and it is included in the request sent to the LLM using the [Chat Completions (with history)](/appstore/modules/genai/commons/#chat-completions-with-history) operation.
