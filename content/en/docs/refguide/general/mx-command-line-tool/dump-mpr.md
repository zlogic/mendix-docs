...
| Option | Value | Result |
| --- | --- | --- |
| `--unit-type` | A single unit type, or a comma-separated list of unit types. If you are looking for a specific unit type, you can check the [Model SDK API documentation](https://apidocs.rnd.mendix.com/modelsdk/latest/index.html). Each unit has a property called `structureTypeName` that contains the unit type. [Here is an example](https://apidocs.rnd.mendix.com/modelsdk/latest/classes/pages.Page.html#structureTypeName-1) for Page document, as in the documentation the unit type is `Pages$Page`. Also we include the unit type in the output JSON (the `$Type` field). So you can use the command without this argument to list the unit types in your project. | Filters the results on the supplied unit types and limits the JSON export. |
