...
* When working on a very big amount of objects, consider to work in batches. Committing or deleting items one by one can be slow, but these activities clean-up memory. Doing this in batches gives the best of both performance and low memory usage. For deletions, this is only possible since Mendix 10.17.0. Use a counter to decide in a loop when to perform the commit or delete activity on the list. Don't forget to do a Clear operation on the `<Entity>_CommitList` list after the commit activity. The `<Entity>_DeleteList` list is cleared automatically during the delete activity.
