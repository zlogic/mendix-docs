...
This guide helps you set up Bitrise to automate building, testing, and deploying Mendix Native apps in the cloud, enabling you to streamline your CI/CD processes.

## Prerequisites {#prerequisites}

Before you begin, ensure you have:

- Installed the [latest MTS version](/releasenotes/studio-pro/lts-mts/#mts) of Mendix Studio Pro using the online installer.
- Completed [Get Started with Native Mobile](/refguide/mobile/getting-started-with-mobile/) to create, style, and debug an application with Mendix Studio Pro.
- Uploaded your native mobile project output to git via Studio Pro and have the cloud address of your uploaded project.
- Accounts for [GitHub](https://github.com/) or other Git provider and [Bitrise](https://bitrise.io).
...
For Android deployment, have an Android device available.
...
### Step 1: Adding Environment Variables in Bitrise

1. Go to your [Bitrise dashboard](https://app.bitrise.io/).
2. Open your project and navigate to **Workflow** > **Env Vars**.
3. Add the following environment variables:

    - **PROJECT_LOCATION**: Location of the Android project (e.g., `android`).
    - **MODULE**: Module name within the Android project (e.g., `app`).
    - **VARIANT**: Build variant for Android (e.g., `appstoreRelease`).
    - **BITRISE_PROJECT_PATH**: Path to the Xcode workspace or project (e.g., `./ios/NativeTemplate.xcworkspace`).
    - **BITRISE_SCHEME**: The scheme name for your iOS project (e.g., `nativeTemplate`).
...
### Step 2: Adding Secrets in Bitrise

Sensitive credentials should be added in the **Secrets** section to keep them secure.
...
3. Ensure **Protect variable** is checked for each secret to keep sensitive information secure.


## Additional Setup: Adding Keystore and Apple Service Connections

To build and deploy iOS and Android applications, you'll need to configure code signing and service connections for Apple and Google services in Bitrise.

### Step 1: Adding a Keystore for Android
...
2. Click **Upload keystore file** and select your `.keystore` file.
3. After uploading, provide the following details:
...
### Step 2: Connecting to Apple Services
...
2. Go to **Workflow** > **Code Signing** > **Apple Service Connection** and select **Add New**.
3. Upload your API key and provide the **Issuer ID** and **Key ID** from your Apple Developer account.
4. After connecting, Bitrise will use this API key for automatic code signing in your iOS builds.

### Step 3: Exporting iOS Code Signing Certificates

If you don’t have your certificates on hand, export them from Xcode:

1. Open **Xcode** and go to **Preferences** > **Accounts**.
2. Select your **Apple ID** and click **Manage Certificates**.
3. Find your **iOS Distribution Certificate**, right-click, and choose **Export Certificate**.
4. Save it as a `.p12` file and upload it to Bitrise under **Workflow** > **Code Signing** > **iOS Certificates**.
...
### Step 4: Connecting a Google Service Account (Optional for Google Services)
...
3. In Bitrise, go to **Workflow** > **Secrets** and add a new secret for the JSON key, such as `GOOGLE_SERVICE_ACCOUNT_KEY`.
4. Follow the Bitrise guide on [Connecting a Google Service Account](https://devcenter.bitrise.io/en/connectivity/connecting-a-google-service-account-to-bitrise.html) for more details.

### Summary

By completing the setup of your keystore, Apple service connection, iOS code signing, and optional Google service account, you will enable Bitrise to handle code signing and integration with necessary services, ensuring smooth builds and deployments for both iOS and Android.
