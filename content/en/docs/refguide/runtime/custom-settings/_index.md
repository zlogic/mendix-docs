...
| <a id="commendixcoreLenientDateTimeParsing" href="#commendixcoreLenientDateTimeParsing">com.<wbr>mendix.<wbr>core.<wbr>LenientDateTimeParsing</a> | The `parseDateTime` function will use more lenient parsing, as it did in Mendix 9, when set to `true`. For example `parseDateTime("yyyyMMdd", "2021-11-10")` will return `2020-11-01` as a date. If set to `false`, which is the default, this example will throw an error. It is recommended to not enable this setting to avoid unexpected results. | `false` |
