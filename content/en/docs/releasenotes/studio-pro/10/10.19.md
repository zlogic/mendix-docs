...
- We fixed a bug in the error checking process that prevented errors for unknown model elements from being removed from the **Errors** pane in Studio Pro.
...
- An XPath collection parameter with a single element now has proper type conversion done like other parameters. This issue affected the workflow engine startup in the case where there is only a single workflow defined in the app. {TW}
