...
- We made the usage of the [variable name](/refguide/java-actions/#variable-name) more consistent when adding Java or JavaScript action calls from the Toolbox, top toolbar, App Explorer, or Logic Recommender.
...
-  We've updated the button labels in the Changes pane and certain dialogs to use "local" and "server" instead of "mine" and "theirs" for selecting the source when resolving file conflicts. {TW}
...
- We introduced a consistency error when the data source is an entity path with steps traversing from a local to a remote entity. (Ticket 203958) {TW}
