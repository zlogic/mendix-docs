...
- We introduced OQL version 2, which contains changes in the OQL engine. The list of changes is available here: [OQL version 2 features](/refguide/oql-v2). By default, OQL v2 is disabled. It can be enabled by setting the [OQL version 2](/refguide/app-settings/#oql-version-2) app setting to *Yes*. You will have to switch to OQL v2 in order to use [view entities](/refguide/view-entities/).
### Online Synchronization Mode (Beta)
We introduced a new feature for offline apps, that allows app developers to define specific entities as "online available". This means that the offline app will execute queries on these entities on the Mendix Runtime Server instead of the
offline database. Of course, this is only possible when you have a connection. This reduces the need to have to synchronize more data then strictly necessary for offline apps as certain parts can become only available when having a connection.
...
- We have added a new feedback message for copy-pasting interrupting boundary events. This message appears when an activity already has the maximum allowed (1) and the user attempts to add another.

### Improvements

- When double-clicking a Consistency Error of an Access Rule in the error panel of Studio Pro we now open the access rule editor with the edit view for this Access Rule. This improvement is for the Beta release of the new access rule editor only.
...
- We improved the XPath constraint syntax, a path ending with an entity can now be compared to an object variable without the need to specify `/id` on the variable. For example, `[OrderLine_Order/Order]=$currentObject` is now a valid constraint.
