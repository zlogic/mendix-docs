...
- We no longer prefix the resulting variable name of list operation actions with "new". Thank you [Maarten Bongers](https://community.mendix.com/link/space/microflows/ideas/3725) for the suggestion!
- It is now possible to select and wrap multiple widgets in a [container](https://docs.mendix.com/refguide/container/) by right-clicking the context menu. (This is based on submitted ideas from various users: [Ceydanur Bakanoğlu](https://community.mendix.com/link/space/studio-pro/ideas/4179), [Bart Heijs](https://community.mendix.com/link/space/studio-pro/ideas/3626) and several others. Thank you all!)
...
- We reduced the time it takes to cancel the process of building an app in Studio Pro. {TW}
...
- The **Integration** pane improved macOS support and features an improved design. It was also updated to use v5 of the Catalog Search API and now lists search results in order of relevancy instead of alphabetically. {TW}
...
- We improved cancellation reactivity when building an application. {TW}
...
- We changed the way that OpenTelemetry is configured for the tracing beta. The project should now be run with the OpenTelemetry Java agent in order to configure and enable tracing. The runtime settings related to tracing were removed. {TW}
- We made the usage of the `ReturnName` when adding Java and JavaScript action calls more consistent between using the toolbox, toolbar, sidebar, and Logic Bot. {TW}
...
- Cancelling while running locally will now happen faster during the "Build deployment structure" phase. {TW}
